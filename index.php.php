...
if (isset($_POST ['add_task'])) {
    ...
    if (!empty($task) && !empty($priority) && !empty($due_date)) {
        mysqli_query($koneksi, "INSERT INTO midun01 VALUES('', '$task', '$priority', '$due_date', '0')");
...
if (isset($_POST['edit_task'])) {
    ...
    mysqli_query($koneksi, "UPDATE midun01 SET task='$task', priority='$priority', due_date='$due_date' WHERE id=$id");
...
if (isset($_GET['complete'])) {
    $id = $_GET['complete'];
    mysqli_query($koneksi, "UPDATE midun01 SET status=1 WHERE id=$id");
    echo "<script>alert('Data berhasil diperbarui');</script>";
    header('Location: index.php');
}
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    mysqli_query($koneksi, "DELETE from midun01 WHERE id=$id");
    echo "<script>alert('Data berhasil dihapus');</script>";
    header('Location: index.php');
}

$result = mysqli_query($koneksi, "SELECT * FROM midun01 ORDER BY status ASC, priority DESC, due_date ASC");
?>
...
<head>
    ...
    <title> hamid ukk </title>
    <link rel="stylesheet" href="style.css"> <!-- Menyambungkan ke file CSS --></head>
<body>
    <div class="container mt-2">
        <h2 class="text-center">Catatan Tahunan</h2>
...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $edit_query = mysqli_query($koneksi, "SELECT * FROM midun01 WHERE id=$id");
